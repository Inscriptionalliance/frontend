...
} from "../view/Market/InscriptionDetail";
import { MintChainBox } from "../view/Mint/Mint";
import { RecordTable_Content_NoData } from "../view/join/joinPerson";
...
const SwapBodyComponentBSC = () => {
  ...
  const SwitchToBtcChain = () => {
    navigate("/View/BTCEnvironment");
  };
        ...
        {false ? (
          ...
          <AddressBox>
            <Dropdown
              overlay={menu}
              placement="bottom"
              overlayClassName="AddressDropDown"
              trigger={["click"]}
              arrow={false}
              getPopupContainer={(triggerNode) => triggerNode}
              onOpenChange={(value) => {
                setSwitchBtcAddressState(value);
              }}
            >
              <DropContaienr_Address>
                <div>
                  {addressBtcArr.find(
                    (item: any) => String(item) === String(CorrentBtcAddress)
                  )
                    ? "BTC1：" +
                      AddrHandle(
                        addressBtcArr.find(
                          (item: any) =>
                            String(item) === String(CorrentBtcAddress)
                        ) as string,
                        6,
                        6
                      )
                ...
                <DropIcon
                  className={
                    SwitchBtcAddressState ? "rotetaOpen" : "rotetaClose"
                  }
                />
              </DropContaienr_Address>
            </Dropdown>
